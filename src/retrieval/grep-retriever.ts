...
export function scorePage(page: WikiPage, query: string, qTokens: string[], agingAfterDays: number, staleAfterDays: number): { score: number; freshness: SearchHit['freshness']; snippet: string; match: MatchStats } | undefined {
  ...
  if (phraseMatched) {
    matched = total;
    if (titleLower.includes(queryLower)) strong = total;
  }
