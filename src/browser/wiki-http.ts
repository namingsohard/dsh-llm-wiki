...
function sendJson(res: WikiHttpResponse, status: number, payload: unknown): void {
...
/** One page row in the tree: enough to draw a list, never the body. */
interface TreePageRow {
  id: string;
  title: string;
  status: string;
  updated: string;
...
function byTitle(a: TreePageRow, b: TreePageRow): number {
 ...
 */
export function createWikiHttpHandlers(store: WikiStore, staging: StagingQueue, config: WikiConfig): WikiHttpHandlers {
  const methodGuard = (req: WikiHttpRequest, res: WikiHttpResponse): boolean => {
  ...
  const queryOf = (req: WikiHttpRequest): URL | undefined => {
    try {
  ...
  const page = async (req: WikiHttpRequest, res: WikiHttpResponse): Promise<void> => {
    if (!methodGuard(req, res)) return;
    const query = queryOf(req);
    const kind = query?.searchParams.get('kind') ?? '';
    const id = query?.searchParams.get('id') ?? '';
    if (!query || !KINDS.includes(kind as PageKind) || !isValidPageId(id)) {
...
function stagedPayload(entry: StagedEntry): Record<string, unknown> {
  return { ...entry };
}

/** The subset of the composition's `connection` service the fence calls. */
interface ConnectionFence {
  requestRejection(req: WikiHttpRequest): number | undefined;
}

/** The subset of the composition's `webServer` service routes register on. */
interface WebServerSeat {
  register(route: { kind: 'exact'; path: string; handler: (req: WikiHttpRequest, res: WikiHttpResponse) => Promise<void> }): () => void;
}
 ...
 */
export function registerWikiHttp(ctx: Context, store: WikiStore, staging: StagingQueue, config: WikiConfig): void {
  ...
  const body = () => {
    ...
    const fence = (): ConnectionFence | undefined => ctx.get('connection', false) as ConnectionFence | undefined;
    const guard = (req: WikiHttpRequest, res: WikiHttpResponse): boolean => {
    ...
    const tryRegister = (): (() => void)[] | undefined => {
      const webServer = ctx.get('webServer', false) as WebServerSeat | undefined;
      if (webServer === undefined) return undefined;
      const registered: (() => void)[] = [];
      const route = (path: string, handler: (req: WikiHttpRequest, res: WikiHttpResponse) => Promise<void>) => {
        registered.push(
          webServer.register({
            kind: 'exact',
            path,
            handler: async (req, res) => {
              ...
              await handler(req, res).catch((error: unknown) => {
                // A settled header means the only honest answer is to drop the socket body.
                if (res.statusCode >= 200 && res.writableEnded !== true) sendJson(res, 500, { ok: false, error: 'wiki-http/internal' });
      ...
      ctx.logger?.info?.(
        'dsh-llm-wiki: wiki browser routes active at %s (root %s, fence %s)',
        WIKI_TREE_ROUTE,
        store.root,
        fence() === undefined ? 'pending' : 'ready',
      );
      return registered;
    };
    ...
    if (disposers === undefined && managed) {
      ...
      const tick = () => {
        ...
        if (Date.now() < deadline) lapseTimer = setTimeout(tick, REGISTER_RETRY_MS);
        else ctx.logger?.debug?.('dsh-llm-wiki: no web surface within %dms — wiki browser routes stay off', REGISTER_WAIT_MS);
      };
  ...
  if (managed) ctx.effect(body, 'dsh-llm-wiki: wiki browser routes');
  else body();
}
