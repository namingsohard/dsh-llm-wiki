...
const PLAYBOOK_FILES = ['router.md', 'extraction.md', 'mutation.md', 'validation.md'] as const;
...
export function createPromptSource(logger?: { warn?: (message: string, ...args: unknown[]) => void }): { provider: (wikiRootDisplay: string) => string; prefetch: () => Promise<void> } {
  const cache: PromptCache = { loaded: false, playbooks: [] };

  const prefetch = async (): Promise<void> => {
    try {
      const texts = await Promise.all(
        PLAYBOOK_FILES.map(async (name) => (await readFile(playbookUrl(name), 'utf8')).trim()),
      );
  ...
  const provider = (wikiRootDisplay: string): string => {
    ...
    if (!cache.loaded || cache.playbooks.length !== PLAYBOOK_FILES.length) return header;
