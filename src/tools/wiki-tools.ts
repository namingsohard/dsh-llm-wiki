...
import type { MutationOp, MutationResult, PageKind } from '../types.js';
import type { WikiApprovalMode, WikiConfig } from '../config.js';
import { assertPageId, slugifyTitle, sourceId } from '../storage/id-slug.js';
import type { WikiStore } from '../storage/markdown-store.js';
import { StagingQueue, type StagedEntry } from '../storage/staging.js';
...
export interface ApprovalSeam {
  request(req: { agent: unknown; toolName: string; callId?: string; reason?: string; signal?: AbortSignal }): Promise<ApprovalOutcome>;
}

/** Registration context: the tool runtime, the optional approval lookup, and a logger. */
export interface ToolsHost {
  tools: {
    register(tool: { name: string }): unknown;
  };
  /**
  ...
  logger?: { warn?: (message: string, ...args: unknown[]) => void; info?: (message: string, ...args: unknown[]) => void } | undefined;
}
...
interface ExecExtras {
  agent?: unknown;
  callId?: string;
  signal?: AbortSignal;
}

/** One `wiki_mutate` result row: what happened to one requested operation. */
interface MutateRow {
  index: number;
  op: string;
  id: string;
...
function approvalNote(outcome: ApprovalOutcome): string {
  switch (outcome) {
    case 'rejected':
...
function summarizeStaged(entry: StagedEntry, index: number): { index: number; id: string; kind: string; op: string; target: string; pitch: string; reason: string } {
  const target = entry.type === 'source'
    ? String((entry.payload as { url?: string; title?: string }).url ?? (entry.payload as { title?: string }).title ?? '')
    : String((entry.payload as MutationOp).id ?? (entry.payload as MutationOp).title ?? '');
  return {
    index,
    id: entry.id,
    kind: entry.type === 'source' ? 'source' : (entry.payload as MutationOp).kind ?? 'concept',
    op: entry.type === 'source' ? 'source_save' : (entry.payload as MutationOp).op,
 ...
 */
function approvalLineFor(entry: StagedEntry): ApprovalLine {
  if (entry.type === 'source') {
    const payload = entry.payload as { title?: string; url?: string };
  ...
  const op = entry.payload as MutationOp & { why?: string };
...
async function askApproval(
  ...
  toolName: string,
  reason: string,
  extras: ExecExtras | undefined,
  fallback: ApprovalOutcome,
): Promise<ApprovalOutcome> {
  const seam = ctx.get?.('approval', false) as ApprovalSeam | undefined;
  if (seam === undefined || typeof seam.request !== 'function' || extras?.agent === undefined) return fallback;
  try {
...
function gateNote(pending: number): string {
...
async function refreshCounts(store: WikiStore, staging: StagingQueue, stats: WikiPromptStats, logger: ToolsHost['logger']): Promise<void> {
  try {
...
const MUTATION_NOTE =
  'Incremental wiki mutation. Batch ALL of a task\'s knowledge changes in one call. Ops: ' +
...
export function registerWikiTools(
   ...
   */
  async function saveSource(id: string, input: { title: string; obtained: string; url?: string }): Promise<{ id: string; staged_as: string; staged: boolean; deduplicated: boolean; bytes: number; note: string }> {
    return await store.withLock(async () => {
      ...
      const page = {
        ...
        tags: [] as string[],
        links: [] as { target: string; relation?: string }[],
        sources: [] as string[],
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        const kinds: PageKind[] =
          args.kind !== undefined
            ? [args.kind as PageKind]
            : ['concept', 'entity', ...((args.include_sources ?? config.includeSourcesInSearch) ? (['source'] as PageKind[]) : [])];
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Record where external material came from: one Source Layer card holding the title, the origin URL and the retrieval time. The original text is NEVER stored here — do not paste page content; the wiki keeps no raw material. What you learned from the material belongs in concept/entity pages, which cite the id this call returns. Same URL deduplicates to the existing card. With the default write gate this only proposes: the entry waits in staging/ for the user.',
      parameters: {
      ...
      async execute(args, exec) {
        ...
        if (gated) {
          ...
          if (config.approval === 'inline') {
            const outcome = await askApproval(ctx, 'wiki_source_save', pitch, exec as ExecExtras, 'unavailable');
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        const inventory = {
          ...
          applied: 0,
          rejected: 0,
          results: [] as { id: string; status: string; detail: string }[],
        };
        ...
        const outcome = await askApproval(ctx, 'wiki_review', pitch, exec as ExecExtras, 'unavailable');
        ...
        for (const entry of selected.filter((item) => item.type === 'source')) {
          const payload = entry.payload as { title: string; obtained: string; url?: string };
          try {
...
function parseObtained(raw: string | undefined): Date {
  if (raw === undefined || raw.trim().length === 0) return new Date();
  const parsed = new Date(raw);
  if (Number.isNaN(parsed.getTime())) throw new Error(`obtained must be an ISO-8601 timestamp (got ${JSON.stringify(raw)})`);
  return parsed;
}
