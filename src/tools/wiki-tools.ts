...
export function registerWikiTools(ctx: ToolsHost, store: WikiStore, mutator: Mutator, config: WikiConfig): void {
  ctx.tools.register(
    defineTool({
      name: 'wiki_search',
      description:
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Persist raw external material (a web-search result you fetched, a document, a passage) into the wiki Source Layer as evidence for later knowledge. Returns the source id to reference in wiki_mutate create/update sources. Same URL or same content hash deduplicates to the existing id. Sources are raw material; distilled knowledge goes into concept/entity pages.',
      parameters: {
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        await store.ensureInit();
        exec.signal?.throwIfAborted();
        const outcome = await mutator.apply(args.operations as unknown as MutationOp[]);
        return {
