...
export function coverageFromScore(topScore: number | undefined, match?: MatchStats): Coverage {
  if (topScore === undefined || topScore < ANY_MIN_SCORE) return 'none';
  const byScore: Coverage = topScore >= HIGH_MIN_SCORE ? 'high' : topScore >= PARTIAL_MIN_SCORE ? 'partial' : 'low';
  // Synthesised hits carry no match stats: fall back to the score alone.
  if (match === undefined || match.total <= 0) return byScore;
  const ratio = match.matched / match.total;
  if (byScore === 'high' && ratio >= HIGH_MIN_RATIO && match.strong >= HIGH_MIN_STRONG) return 'high';
  if (topScore >= PARTIAL_MIN_SCORE && ratio >= PARTIAL_MIN_RATIO) return 'partial';
  return 'low';
}
...
function matchNote(match: MatchStats | undefined): string {
  if (match === undefined || match.total <= 0) return '';
...
export function routeQuery(hits: readonly SearchHit[], query: string): RouterDecision {
  ...
  let advice: string;
  let nextStep: string;
  let guideTopic: RouterDecision['guideTopic'];
  if (coverage === 'high') {
    const id = top?.id ?? '';
  ...
  const decision: RouterDecision = { coverage, useWiki, needWeb, advice, nextStep, guideTopic };
