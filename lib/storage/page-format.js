...
function scalarOut(value) {
    if (value.length === 0)
        return '""';
    if (BARE_TOKEN.test(value) && !/^\s|\s$/.test(value))
        return value;
    return JSON.stringify(value);
}
function arrayOut(values) {
    if (values.length === 0)
        return '[]';
...
function splitInlineArray(body, context) {
    const out = [];
    let current = '';
    let inQuotes = false;
    let escaped = false;
    ...
    if (trimmed.length > 0)
        out.push(trimmed);
    if (inQuotes)
        throw new PageFormatError(`${context}: unterminated quote in inline array`);
    return out.map((item) => decodeScalar(item, context));
}
function decodeScalar(raw, context) {
...
export function serializePage(meta, body) {
    ...
    lines.push(`links: ${arrayOut(meta.links.map((link) => (link.relation === undefined || link.relation.length === 0 ? link.target : `${link.target} | ${link.relation}`)))}`);
    lines.push(`sources: ${arrayOut(meta.sources)}`);
    if (meta.url !== undefined)
        lines.push(`url: ${scalarOut(meta.url)}`);
    if (meta.obtained !== undefined)
        lines.push(`obtained: ${scalarOut(meta.obtained)}`);
    if (meta.supersededBy !== undefined)
        lines.push(`superseded_by: ${scalarOut(meta.supersededBy)}`);
    lines.push('---');
    return `${lines.join('\n')}\n${body}`;
}
/** Parse page text back into a {@link WikiPage}. Unknown frontmatter keys are ignored. */
export function parsePage(text, path) {
    const context = `wiki page ${path}`;
    const lines = text.split(/\r?\n/);
    if (lines[0] !== '---')
        throw new PageFormatError(`${context}: missing frontmatter`);
    const end = lines.indexOf('---', 1);
    if (end < 0)
        throw new PageFormatError(`${context}: unterminated frontmatter`);
    ...
    for (const line of lines.slice(1, end)) {
        ...
        if (match === null)
            throw new PageFormatError(`${context}: bad frontmatter line ${JSON.stringify(line)}`);
        const key = match[1];
    ...
    if (!KINDS.includes(kind))
        throw new PageFormatError(`${context}: unknown kind ${JSON.stringify(kind)}`);
    if (!STATUSES.includes(status))
        throw new PageFormatError(`${context}: unknown status ${JSON.stringify(status)}`);
    try {
        assertPageId(id);
    }
    catch (error) {
        throw new PageFormatError(`${context}: ${error.message}`);
    }
    const links = (arrays.get('links') ?? []).map((entry) => {
        const bar = entry.indexOf('|');
        if (bar < 0)
            return { target: entry.trim() };
...
//# sourceMappingURL=page-format.js.map