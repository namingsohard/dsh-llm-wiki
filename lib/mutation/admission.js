...
const DIMENSION_KEYS = ['reusability', 'stability', 'novelty', 'abstraction'];
/** Validate raw scores; throws with a precise message on malformed input. */
export function assertAdmissionScores(raw) {
    if (typeof raw !== 'object' || raw === null)
        throw new Error('admission must be an object with the four score dimensions');
    const record = raw;
    const out = {};
...
export function evaluateAdmission(scores, config) {
    ...
    if (average < config.admissionMinAverage)
        reasons.push(`average ${average.toFixed(2)} below threshold ${config.admissionMinAverage}`);
