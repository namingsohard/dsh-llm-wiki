...
export function clampInline(text, max) {
...
export function renderOpPitch(op, detail) {
    ...
    switch (op.op) {
        ...
        case 'update': {
            ...
            if (op.body !== undefined)
                what.push(`body → ${clampInline(op.body, PITCH_BODY_CHARS)}`);
            if (op.title !== undefined)
                what.push(`title → ${op.title}`);
            if (op.status !== undefined)
                what.push(`status → ${op.status}`);
            if (op.tags !== undefined)
                what.push(`tags += ${list(op.tags)}`);
            if (op.links !== undefined)
                what.push(`links += ${list(op.links)}`);
            if (op.sources !== undefined)
                what.push(`sources += ${list(op.sources)}`);
...
export function renderSourcePitch(title, url) {
 ...
 */
export function renderApprovalPitch(items) {
    ...
    if (items.length > shown.length)
        parts.push(`+${(items.length - shown.length).toString()} more, listed in the wiki_review result`);
