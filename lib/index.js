...
export function apply(ctx, config) {
    ...
    void store.ensureInit().then(async () => {
        await store.rebuildIndex();
        await tools.refresh();
    }, (error) => ctx.logger?.warn?.('dsh-llm-wiki: wiki root initialization failed: %s', error instanceof Error ? error.message : String(error)));
